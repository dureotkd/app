		...
		<tbody>
			...
			<c:if test="${article.extra.file__common__attachment['4'] != null}">
				<tr>
					<th>첨부 파일 4 이미지</th>
					<td>
						<div class="img-box"><img src="/usr/file/showImg?id=${article.extra.file__common__attachment['4'].id}&updateDate=${article.extra.file__common__attachment['4'].updateDate}" /></div>
					</td>
				</tr>
...
<div class="reply-modify-form-modal flex flex-ai-c flex-jc-c">
	<form action="" class="form1 bg-white padding-10"
		...
		<div class="form-row">
			...
			<div class="video-box video-box-file-3"></div>
		</div>
		<div class="form-row">
...
<script>
	...
	function ReplyList__submitModifyForm(form) {
		...
		var startUploadFiles = function() {
			if (fileInput1.value.length == 0 && fileInput2.value.length == 0 && fileInput3.value.length == 0 ) {
				if (deleteFileInput1.checked == false
						&& deleteFileInput2.checked == false && deleteFileInput3.checked == false) {
		...
		var onModifyReplyComplete = function(data) {
			if (data.resultCode && data.resultCode.substr(0, 3) == 'S-') {
				...
				if ( data && data.body && data.body.file__common__attachment ) {
					...
					for ( var fileNo = 3; fileNo <= 4; fileNo++ ) {
						var file = data.body.file__common__attachment[fileNo];
	...
	function ReplyList__showModifyFormModal(el) {
		...
		for ( var fileNo = 3; fileNo <= 4; fileNo++ ){ 
			...
			var $videoBox = $('.reply-list-box [data-video-name="' + imgName + '"]');
	...
	function ReplyList__drawReply(reply) {
		...
		for ( var fileNo = 3; fileNo <= 4; fileNo++){
			...
			if ( reply.extra.file__common__attachment && reply.extra.file__common__attachment[fileNo] ) {
				...
				html += '<img src="/usr/file/showImg?id=' + file.id + '&updateDate=' + file.updateDate + '"></img>';
	        } else {
		        }
