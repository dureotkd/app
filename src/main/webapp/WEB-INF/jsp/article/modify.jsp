...
<script>
	...
	function ArticleModifyForm__submit(form) {
		...
		var fileInput4 = form["file__article__" + param.id
			+ "__common__attachment__4"]
		...
		var deleteFileInput3 = form["deleteFile__article__" + param.id
			+ "__common__attachment__3"];
		var deleteFileInput4 = form["deleteFile__article__" + param.id
			+ "__common__attachment__4"];
		...
		if (fileInput4.value) {
			if (fileInput4.files[0].size > maxSize) {
				alert(maxSizeMb + "MB 이하의 파일을 업로드 해주세요.");
				return;
			}
		}
		...
		var startUploadFiles = function(onSuccess) {
			if (fileInput1.value.length == 0 && fileInput2.value.length == 0 && fileInput3.value.length == 0 && fileInput4.value.length == 0 ) {
				if (deleteFileInput1.checked == false && deleteFileInput2.checked == false && deleteFileInput3.checked == false && deleteFileInput4.checked == false  ) {
					onSuccess();
					return;
				}
			}
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<div class="form-control-box">
						<input type="file" accept="img/*"
							name="file__article__${article.id}__common__attachment__4" />
					</div> <c:if
						test="${article.extra.file__common__attachment['4'] != null}">
						<div class="video-box">
							<img 
								src="/usr/file/showImg?id=${article.extra.file__common__attachment['4'].id}&updateDate=${article.extra.file__common__attachment['4'].updateDate}">
							</img>
						</div>
					</c:if>
				</td>
			...
			<tr>
				...
				<td>
					<div class="form-control-box">
						<label><input type="checkbox"
							name="deleteFile__article__${article.id}__common__attachment__4"
							value="Y" /> 삭제 </label>
					</div>
				</td>
			</tr>
